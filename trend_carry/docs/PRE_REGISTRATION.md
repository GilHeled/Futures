# PRE-REGISTRATION (FROZEN) — Diversified Trend + Carry (Futures)

> **Status: FROZEN — 2026-07-27.** Approved by the user and locked. This is the immutable
> pre-registered record. All design decisions (§12) are fixed; parameters, universe,
> signals, portfolio construction, costs, splits, and Go/No-Go thresholds are not revised
> after seeing results. The only permitted changes are corrections to an implementation
> **bug** or a **violation of this protocol** — each of which must be documented, never
> silently applied. Development is a single pass; the locked hold-out (2025-01-01 →
> 2026-07-09) is touched exactly once, and only if the development/OOS Go/No-Go passes.
> Trend (Phase 1) and carry (Phase 2) are independent pre-registered existence tests; the
> line closes only if both fail (§9). The living design doc is `DESIGN_AND_PREREG.md`;
> this file is the frozen snapshot at approval.
