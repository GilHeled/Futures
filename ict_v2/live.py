...
class V2Live:
    ...
    def _setup_dict(mtf) -> Optional[dict]:
        ...
        return {"available": len(mtf.candidates), "gated": len(mtf.gated), "total": len(ci),
                ...
                # the four-layer RECOMMENDATION tally (TAKE = valid + all course filters pass)
                "take": sum(1 for x in ci if x.get("recommendation") == "TAKE"),
                "skip": sum(1 for x in ci if x.get("recommendation") == "SKIP"),
                "watch": sum(1 for x in ci if x.get("recommendation") == "WATCH"),
    ...
    def snapshot(self) -> dict:
        ...
        return {
            ...
            "execution": None if not e else {
                ...
                "take": sum(1 for x in e.cand_info if x.get("recommendation") == "TAKE"),
                "skip": sum(1 for x in e.cand_info if x.get("recommendation") == "SKIP"),
                "watch": sum(1 for x in e.cand_info if x.get("recommendation") == "WATCH"),
