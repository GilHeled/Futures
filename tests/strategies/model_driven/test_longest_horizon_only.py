...
from mnq_system.strategies.model_driven.longest_horizon_only import LongestHorizonOnlyStrategy
from tests.strategies.model_driven._fixtures import account, fast_model_driven_config, make_multi_day_bars


def test_only_the_longest_horizon_is_consulted():
    cfg = fast_model_driven_config(confidence_threshold=0.0)
    strategy = LongestHorizonOnlyStrategy(cfg, account())

    signal = strategy.combine_horizon_signals(
        horizon_directions={10: -1, 20: -1, 40: 1}, horizon_confidences={10: 0.9, 20: 0.9, 40: 0.9}
    )

    assert signal == "long"  # h10/h20 disagree, but only h40 (the longest horizon) is ever consulted


def test_stands_aside_when_the_longest_horizon_is_flat():
    cfg = fast_model_driven_config(confidence_threshold=0.0)
    strategy = LongestHorizonOnlyStrategy(cfg, account())

    signal = strategy.combine_horizon_signals(
        horizon_directions={10: 1, 20: 1, 40: 0}, horizon_confidences={10: 0.9, 20: 0.9, 40: 0.9}
    )

    assert signal is None


def test_stands_aside_when_the_longest_horizon_fails_the_shared_threshold():
    cfg = fast_model_driven_config(confidence_threshold=0.9)
    strategy = LongestHorizonOnlyStrategy(cfg, account())

    signal = strategy.combine_horizon_signals(
        horizon_directions={10: 1, 20: 1, 40: 1}, horizon_confidences={10: 0.9, 20: 0.9, 40: 0.5}
    )
...
def test_engine_end_to_end_produces_valid_trades():
    ...
    strategy = LongestHorizonOnlyStrategy(cfg, acct)
    engine = BacktestEngine({"entry": bars}, strategy, acct, BacktestSettings(account_equity=50_000.0))
    result = engine.run()

    assert len(result.trades) > 0
    assert all(t.setup_type == "model_longest_horizon" for t in result.trades)
...
def test_decisions_up_to_a_cutoff_are_unaffected_by_changing_bars_after_it():
    ...
    result_a = BacktestEngine(
        {"entry": bars_a}, LongestHorizonOnlyStrategy(cfg, acct), acct, BacktestSettings(account_equity=50_000.0)
    ).run()
    result_b = BacktestEngine(
        {"entry": bars_b}, LongestHorizonOnlyStrategy(cfg, acct), acct, BacktestSettings(account_equity=50_000.0)
    ).run()
